...
void write_max2 (uint8_t address1, uint8_t data1, uint8_t address2, uint8_t data2, uint8_t address3, uint8_t data3, uint8_t address4, uint8_t data4)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		for (int j=0; j<38; j++)
		{
			for (int i=1; i<9; i++)
			{
				write_max2(i, disp1ay[j][i-1], i, disp1ay[37-j][i-1], i, disp1ay[j][i-1], i, disp1ay[37-j][i-1]);
//				write_max2(i , disp1ay[37-j][i-1]);
			}
			HAL_Delay (700);
		}

//		write_string ("CONTROLLERSTECH");
//		HAL_Delay (1000);
  }
  /* USER CODE END 3 */
}
